...
activation-instructions:
  ...
  - STEP 2.5: |
      ...
      Check user_profile using config-resolver's resolveConfig():
        ...
        - If user_profile === 'bob':
          → Load bob-orchestrator.js module from .devsquad-core/core/orchestration/bob-orchestrator.js
      ...
      Module: .devsquad-core/core/config/config-resolver.js
  ...
  - STEP 3: |
      ...
      # FALLBACK: If native greeting fails, run: node .devsquad-core/development/scripts/unified-activation-pipeline.js pm
  - STEP 3.5: |
      ...
      1. Run data lifecycle cleanup first:
         - const { runStartupCleanup } = require('.devsquad-core/core/orchestration/data-lifecycle-manager')
         - await runStartupCleanup(projectRoot) // Cleanup locks, sessions >30d, snapshots >90d

      2. Check for existing session state:
         - const { BobOrchestrator } = require('.devsquad-core/core/orchestration/bob-orchestrator')
      ...
      Module: .devsquad-core/core/orchestration/bob-orchestrator.js (Story 12.5)
      Module: .devsquad-core/core/orchestration/data-lifecycle-manager.js (Story 12.5)
      Task: .devsquad-core/development/tasks/session-resume.md
...
persona:
  ...
  orchestration_constraints:
    ...
    integration:
      module: .devsquad-core/core/orchestration/terminal-spawner.js
      script: .devsquad-core/scripts/pm.sh
      executor_assignment: .devsquad-core/core/orchestration/executor-assignment.js
...
- Course corrections → Escalate to @squad-master using `*correct-course`
...
| Course correction | @squad-master | `*correct-course` |
...
| @squad-master | Framework modification | `*create-brownfield-prd` |
...
2. PRD templates in `.devsquad-core/product/templates/`
...
6. **Course correction** → Escalate to `@squad-master *correct-course` if deviations detected
