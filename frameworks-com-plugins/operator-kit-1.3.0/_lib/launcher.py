...
def resolve_python_launcher(repo_root: Path | None = None) -> str:
    ...
    for nm in names:
        ...
        if p:
            return p
    raise RuntimeError("nenhum Python encontrado (nem venv, nem sys.executable, nem PATH)")
...
def _self_test() -> int:
    ...
    assert launcher, "launcher vazio"
    name = Path(launcher).name.lower()
    if os.name != "nt":
        assert name not in ("py", "py.exe"), f"launcher 'py' fora do Windows: {launcher}"
    assert Path(launcher).exists(), f"launcher resolvido não existe: {launcher}"

    root = resolve_repo_root()
    assert isinstance(root, Path) and root.exists(), f"repo_root inválido: {root}"
    ...
    try:
        ...
        assert found_root == tmp, f"busca por .git subindo falhou: {found_root} != {tmp}"
    finally:
