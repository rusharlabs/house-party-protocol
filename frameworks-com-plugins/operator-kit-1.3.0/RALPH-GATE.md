...
O plugin oficial `ralph-loop` (Anthropic) tem a mecânica certa: um Stop hook `decision:block`
que re-alimenta o prompt até o modelo emitir `<promise>TEXTO</promise>`. Mas o "done" ali é
**auto-declarado por tag de texto, sem rodar nada** — o `stop-hook.sh` compara string e libera.
...
Por que NÃO usar o plugin oficial como motor por padrão:
- **DOA em máquinas sem `jq`** — `stop-hook.sh` usa `jq` em 3 pontos + `set -euo pipefail`;
  sem `jq` no PATH, crash silencioso já na 1ª iteração (verificado nesta máquina 2026-07-10).
...
```
goal_ledger.py --next  →  /ralph-gate --goal G-X --criteria "<DoD do PRD>" --charter "..."
   → done_gate passa  →  --set G-X done  →  goal_review.py --goal G-X  →  próximo goal
```
