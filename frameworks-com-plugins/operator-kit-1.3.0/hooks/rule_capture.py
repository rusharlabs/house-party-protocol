...
def build_entry(prompt: str, hits: list[str], now: str) -> str:
    ...
    quoted = "\n".join(f"> {ln}" for ln in prompt.splitlines()) or "> (vazio)"
    return (
        f"\n## {now} — captura (marcadores: {', '.join(hits)})\n"
        f"{quoted}\n"
    )
...
def _ensure_header(capture_file: Path) -> None:
    ...
    capture_file.write_text(
        "# Regras CAPTURADAS (verbatim) — gatilho REGRA #10\n\n"
        "> Anexado pelo `rule_capture.py` (Operator Kit) quando o operador usa um\n"
        "> marcador de enfase (SEMPRE/NUNCA/...). Texto LITERAL, nunca interpretado.\n"
        "> Revise periodicamente e promova a regra canonica o que fizer sentido.\n",
        encoding="utf-8",
    )
    ...
    try:
        ...
        if prompt:
            ...
            if ativo:
                ...
                if hits:
                    print(
                        f"[rule_capture] instrucao enfatica capturada (marcadores: {', '.join(hits)}) "
                        f"-> {capture_file}",
                        file=sys.stderr,
                    )
...
def _self_test() -> None:
    ...
    assert detect_markers("processar batch 3", marc) == [], "sem marcador nao deve detectar"
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert hits == ["NUNCA"], f"esperava deteccao de NUNCA, veio {hits}"
        assert cap.exists(), "arquivo de captura deveria ser criado"
        body = cap.read_text(encoding="utf-8")
        assert "> NUNCA faca git push direto na main" in body, "texto literal deveria ser anexado"
        assert "captura" in body

        # de-dup: mesma instrucao nao re-anexa
        capture("NUNCA faca git push direto na main", cap, marc)
        body2 = cap.read_text(encoding="utf-8")
        assert body2.count("> NUNCA faca git push direto na main") == 1, "nao deveria duplicar"
        ...
        assert len(cap.read_text(encoding="utf-8")) == n_antes, "sem marcador, sem escrita"
