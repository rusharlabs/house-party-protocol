...
def _warn(exp: str, tgt: str) -> None:
    sys.stderr.write(
        "[project_root_confirm] AVISO: alvo fora da raiz fixada — possível cross-project.\n"
        f"  raiz esperada: {exp}\n"
        f"  raiz do alvo : {tgt}\n"
        "  -> Confirme que é o repo certo. Se for intencional, defina ALLOW_CROSS_ROOT=1. "
...
def _self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert v1 is None, f"alvo interno não deveria avisar: {v1}"
        # 2. alvo em OUTRO repo -> avisa
        v2 = evaluate({"file_path": str(repo_b / "y.md")}, repo_a, prof)
        assert v2 is not None, "alvo cross-root deveria avisar"
        ...
        assert v3 is None, f"sem âncora deveria ser None: {v3}"
        # 4. env EXPECTED_ROOT sobrepõe
        os.environ["EXPECTED_ROOT"] = str(repo_a)
        try:
            v4 = evaluate({"file_path": str(repo_b / "y.md")}, base, {})
            assert v4 is not None, "env EXPECTED_ROOT deveria ancorar e avisar"
        finally:
        ...
        assert _norm(tdv) == _norm(repo_a / "sub"), f"target_dir relativo errado: {tdv}"
    print("self-test OK")
