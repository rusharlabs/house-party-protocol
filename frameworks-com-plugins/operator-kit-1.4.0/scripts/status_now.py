...
def extract_status(text: str) -> dict:
        ...
        for ln in lines:
            ...
            if s.startswith("#"):
                ...
                if h:
                    ...
                    break
    if etapa is _NA:
        notas.append("etapa nao encontrada no SSoT (sem heading/rotulo Fase|Etapa)")
    ...
    if mpct:
        ...
        notas.append("progresso % nao encontrado no SSoT")

    # BLOQUEIOS / PROXIMA ACAO: pendencias abertas '- [ ]'
    for ln in lines:
        m = _RE_OPEN_TODO.match(ln)
        if m:
            open_todos.append((m.group(1) or "(sem descricao)").strip())

    proxima = open_todos[0] if open_todos else _NA
    if not open_todos:
        notas.append("nenhuma pendencia aberta '- [ ]' no SSoT (proxima acao indefinida)")
...
def build_status(start: Path | None = None) -> dict:
    ...
    if not profile:
        data["notas"].append("operator-profile.yaml ausente (ou PyYAML indisponivel) — configure paths.state_ssot")
        return data
    if not ssot_rel:
        data["notas"].append("paths.state_ssot nao definido no operator-profile.yaml")
        return data
    if ssot is None or not ssot.exists():
        data["notas"].append(f"SSoT nao encontrado no disco: {ssot_rel}")
        return data

    try:
        text = ssot.read_text(encoding="utf-8", errors="replace")
    except Exception as e:  # noqa: BLE001 — nunca crasha
        data["notas"].append(f"falha ao ler SSoT: {e}")
        return data
...
def render_template(data: dict) -> str:
    """Renderiza o template fixo 'onde estamos?' (pt-BR)."""
    ...
    linhas = [
        ...
        f"  ETAPA       : {etapa}",
        f"  PROGRESSO   : {pct}",
        f"  BLOQUEIOS   : {blq} pendencia(s) aberta(s)" if blq != _NA else f"  BLOQUEIOS   : {_NA}",
        f"  PROXIMA ACAO: {prox}",
        f"  FONTE (SSoT): {ssot}",
    ]
    if notas:
        linhas.append("  ---")
        linhas.append("  NOTAS (campos '--' = nao encontrado, NAO inventado):")
        for n in notas:
            linhas.append(f"    - {n}")
    return "\n".join(linhas)
...
def _self_test() -> None:
    ...
    ssot_text = (
        "# Projeto Demo\n"
        "## Fase 2 - Pipeline\n"
        "Progresso atual: 37% concluido\n\n"
        "- [x] Setup inicial\n"
        "- [ ] Processar BATCH-004\n"
        "- [ ] Validar dossiers\n"
    )
    parsed = extract_status(ssot_text)
    assert parsed["etapa"] == "Fase 2 - Pipeline", parsed["etapa"]
    assert parsed["progresso"] == "37%", parsed["progresso"]
    assert parsed["bloqueios_abertos"] == 2, parsed["bloqueios_abertos"]
    assert parsed["proxima_acao"] == "Processar BATCH-004", parsed["proxima_acao"]

    # fixture vazia: tudo '--' + notas (NUNCA inventa)
    parsed2 = extract_status("texto sem nada estruturado\n")
    ...
    data_full = {
        "projeto": "demo", "ssot_rel": "x.md", "etapa": "Fase 2",
        "progresso": "37%", "bloqueios_abertos": 2, "proxima_acao": "fazer X", "notas": [],
    }
    out_full = render_template(data_full)
    assert "Fase 2" in out_full and "37%" in out_full and "fazer X" in out_full
