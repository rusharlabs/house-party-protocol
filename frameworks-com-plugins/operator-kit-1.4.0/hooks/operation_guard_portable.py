...
def assess(command: str, profile: dict | None = None) -> dict:
    ...
    if block_on("git-push-force") and _GIT_PUSH_FORCE.search(cmd):
        return {
            ...
            "reason": ("git push --force reescreve histórico remoto. PROIBIDO "
                       "(use --force-with-lease só com aprovação explícita do operador)."),
        }
    ...
    if block_on("drop-truncate"):
        if _SQL_DROP.search(cmd):
            return {"action": BLOCK, "rule": "drop-truncate",
                    "reason": "DROP TABLE/DATABASE/SCHEMA destrói dados. ESCALAR ao operador."}
        if _SQL_TRUNCATE.search(cmd):
            return {"action": BLOCK, "rule": "drop-truncate",
                    "reason": "TRUNCATE esvazia a tabela inteira. ESCALAR ao operador."}
        if _SQL_DELETE_NO_WHERE.search(cmd):
            return {"action": BLOCK, "rule": "drop-truncate",
                    "reason": "DELETE FROM sem WHERE apaga a tabela inteira. Adicione WHERE."}
    ...
    if _GIT_PUSH.search(cmd) and not _GIT_PUSH_FORCE.search(cmd):
        ...
        if warn_on("git-push-pr-only") or warn_on("git-push-feature"):
            return {
                ...
                "reason": ("git push de feature branch — confirme que vai via PR e não direto na main. "
                           "Branch + PR + merge."),
            }
    ...
    if warn_on("npm-floating-specifier"):
        ...
        if (is_npm or is_pip) and _FLOATING_SPEC.search(cmd):
            ...
            return {
                ...
                "reason": (f"{mgr} install com specifier flutuante (@latest/^/~/*) — fixe a versão "
                           "exata + verifique pacote/hash (feedback_npm_install_security)."),
            }

    return {"action": ALLOW, "rule": "", "reason": "sem regra de risco casada"}
...
def _self_test() -> None:
    ...
    # ---- BLOCK ----
    assert assess("rm -rf src", prof)["action"] == BLOCK, "rm -rf src deveria BLOCK"
    ...
    assert assess("git push origin main", prof)["action"] == BLOCK, "push main deveria BLOCK"
    ...
    # ---- WARN ----
    assert assess("git push origin feat/x", prof)["action"] == WARN, "push feature deveria WARN"
    assert assess("git push origin feat/x", prof)["rule"] == "git-push-pr-only"
    assert assess("npm install lodash@latest", prof)["action"] == WARN, "npm @latest deveria WARN"
    ...
    # ---- ALLOW ----
    assert assess("echo hello", prof)["action"] == ALLOW, "echo deveria ALLOW"
    assert assess("ls -la", prof)["action"] == ALLOW
    assert assess("git status", prof)["action"] == ALLOW
    assert assess("rm -rf /tmp/cache", prof)["action"] == ALLOW, "rm fora de path protegido = ALLOW"
    assert assess("npm install lodash@4.17.21", prof)["action"] == ALLOW, "versão fixa = ALLOW"
    assert assess("delete from jobs where id=1", prof)["action"] == ALLOW, "DELETE com WHERE = ALLOW"
    ...
    assert assess("rm -rf src", only_force)["action"] == ALLOW, "família desligada não dispara"
