...
def hook_stop(payload: dict) -> dict:
    ...
    if not m:
        ...
        return {
            ...
            "systemMessage": f"ralph-gate: iteração {state['iteration']} (sem <promise> detectada)",
        }
    ...
    return {
        "decision": "block",
        "reason": f"{state['charter']}\n\nNÃO emita <promise> até passar DE VERDADE. Falhas reais:\n{tails}",
        "systemMessage": "ralph-gate: done_gate REJEITOU a promise (critério falhou de verdade)",
    }
...
def cmd_start(args) -> int:
    ...
    save_state(state)
    print(f"ralph-gate: armado (goal={args.goal}, max_iterations={args.max_iterations}, {len(args.criteria)} critério(s))")
    return 0


def cmd_status() -> int:
    state = load_state()
    print(json.dumps(state, indent=2, ensure_ascii=False) if state else "ralph-gate: nenhum loop ativo")
    return 0
...
def cmd_cancel() -> int:
    ...
    remove_state()
    print("ralph-gate: cancelado" if had else "ralph-gate: nenhum loop ativo")
    return 0
...
def _self_test() -> int:
    ...
    try:
        ...
        write_transcript("ainda trabalhando, sem tag nenhuma")
        out = hook_stop({"session_id": "s1", "transcript_path": str(transcript)})
        assert out.get("decision") == "block", f"T1 esperava block: {out}"
        assert load_state()["iteration"] == 1, "T1: iteration deveria incrementar"
        ...
        out = hook_stop({"session_id": "s1", "transcript_path": str(transcript)})
        assert out.get("decision") == "block", f"T1b esperava block (critério falso): {out}"
        assert load_state() is not None, "T1b: state deveria sobreviver ao FAIL"

        # T2: promise + critério PASSA -> {} (permite parar), state removido, ledger tem gate-passed
        save_state({"session_id": "s1", "charter": "continue", "criteria": [f'"{sys.executable}" -c "pass"'], "max_iterations": 0, "iteration": 0})
        write_transcript("terminei <promise>DONE</promise>")
        out = hook_stop({"session_id": "s1", "transcript_path": str(transcript)})
        assert out == {}, f"T2 esperava {{}} (permitir parar): {out}"
        assert load_state() is None, "T2: state deveria ser removido"
        ledger_lines = LEDGER_PATH.read_text(encoding="utf-8").splitlines()
        assert any(json.loads(l)["event"] == "gate-passed" for l in ledger_lines), "T2: ledger sem gate-passed"
        ...
        out = hook_stop({"session_id": "s1", "transcript_path": str(transcript)})
        assert out.get("decision") == "block", f"T3: texto forjado NÃO deveria enganar o gate: {out}"
        assert nonce.exists(), "T3: nonce deveria existir — prova que o critério RODOU de verdade no processo do hook"
        ...
        out = hook_stop({"session_id": "s1", "transcript_path": str(transcript)})
        assert out == {}, f"T4 esperava {{}} (teto atingido, libera): {out}"
        assert load_state() is None, "T4: state deveria ser removido no teto"
        ledger_lines = LEDGER_PATH.read_text(encoding="utf-8").splitlines()
        assert any(json.loads(l)["event"] == "paused-budget" for l in ledger_lines), "T4: ledger sem paused-budget"

        # isolamento de sessão: loop de outra sessão não é tocado
        save_state({"session_id": "s1", "charter": "continue", "criteria": [f'"{sys.executable}" -c "pass"'], "max_iterations": 0, "iteration": 0})
        out = hook_stop({"session_id": "s2-outra-sessao", "transcript_path": str(transcript)})
        assert out == {}, f"isolamento de sessão falhou: {out}"
        assert load_state() is not None, "isolamento de sessão: não deveria ter tocado o state de s1"

        print("self-test OK — T1 (mentira=block), T1b (critério falso=block), T2 (done real=libera+ledger), "
              "T3 (forja+nonce=prova execução real), T4 (teto=paused-budget), isolamento de sessão")
        return 0
    finally:
