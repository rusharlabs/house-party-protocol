...
def render(gates: list[str], src: str) -> str:
    if not gates:
        return f"✅ GATE-SHEET vazia (nenhum gate humano aberto)\n   fonte: {src}"
    lines = [f"🔴 GATE-SHEET — {len(gates)} item(ns) esperando o humano", f"   fonte: {src}", ""]
...
def main(argv) -> int:
    ...
    if sheet is None or not sheet.exists():
        print(f"gate_sheet_panel: gate-sheet não encontrada ({sheet}) — defina paths.gate_sheet", file=sys.stderr)
        return 0
    try:
...
def _self_test() -> None:
    ...
    sample = (
        ...
        "## FORMULÁRIO GATE HUMANO\n| Gate | Comando |\n|---|---|\n| billing | abrir painel |\n"
        "🔴 rotacionar secret CF\ntexto comum ignorado\n"
    )
    gates = extract_gates(sample)
    assert "decisão pendente do operador" in gates, gates
    assert any("billing" in g for g in gates), gates
    assert any("rotacionar secret" in g for g in gates), gates
    assert "texto comum ignorado" not in gates
    assert "feito" not in gates  # [x] não é gate aberto
    with tempfile.TemporaryDirectory() as d:
        f = Path(d) / "gates.md"
        f.write_text(sample, encoding="utf-8")
        assert "3 item" in render(extract_gates(f.read_text(encoding="utf-8")), str(f))
    assert "vazia" in render([], "x")
    print("self-test OK")
