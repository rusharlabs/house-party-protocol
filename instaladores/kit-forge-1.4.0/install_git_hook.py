...
stdlib only. v1.0.0 — 2026-07-10 (FASE 1 · kit-forge)
"""
...
def install(repo: Path, hook_name: str, payload_path: Path) -> tuple:
    ...
    if not hooks_dir.is_dir():
        return 3, {"status": "error", "errors": [f".git/hooks não existe em {repo}"]}
    if not payload_path.exists():
        return 3, {"status": "error", "errors": [f"payload não existe: {payload_path}"]}
...
def _self_test() -> int:
    ...
    try:
        ...
        assert code1 == 0 and report1["status"] == "installed", f"instalação simples falhou: {report1}"
        ...
        assert code2 == 0 and report2["status"] == "chained", f"chain deveria acontecer: {report2}"
        ...
        assert "original-ran" in log_content, f"original NUNCA deveria parar de rodar: {log_content!r} (stderr={result.stderr!r})"
        assert "payload2-ran" in log_content, f"payload deveria rodar após o original: {log_content!r}"
        assert log_content.index("original-ran") < log_content.index("payload2-ran"), "ordem errada: original deveria rodar ANTES do payload"

        code2b, report2b = install(repo2, "pre-commit", payload2)
        assert code2b == 0 and report2b["status"] == "no-op", f"2a instalação (já encadeado, payload igual) deveria ser no-op: {report2b}"

        payload2.write_text('#!/bin/sh\necho payload2-v2-ran >> "$1"\n', encoding="utf-8")
        code2c, report2c = install(repo2, "pre-commit", payload2)
        assert code2c == 0 and report2c["status"] == "payload-updated", f"payload mudou, deveria atualizar: {report2c}"

        print("self-test OK — instalação simples, chain-preserving (original roda antes), idempotência e atualização de payload cobertos")
        return 0
    finally:
...
def main(argv) -> int:
    ...
    if not args.repo or not args.payload:
        print("uso: install_git_hook.py --repo <dir> --hook-name pre-commit --payload <script.sh>", file=sys.stderr)
        return 3
