...
Na distribuição emitida este módulo vive em `instaladores/kit-forge-1.4.0/` (o diretório
...
```bash
KIT=instaladores/kit-forge-1.4.0
...
```
```
self-test OK — verify (ok/warn/corrupt/error) + compat posicional + install (6 estágios: detect/prereqs/profile/configure/wire/smoke, plan-first/--apply, --answers, re-run) + registry (kit+target, sem duplicar) + render_plan + marketplace (fora-do-lugar/versão-divergente + controle)
```
...
```
```
self-test OK — wire idempotente, conflito sem --force preservado, --force sobrescreve, --undo byte-idêntico, queda no meio não trunca, escrita concorrente recusa (exit 2)
```
<!-- executado: 2026-09-21 · exit=0 -->
