...
def _kit_name(kit_dir: Path) -> str:
    ...
    if not _valid_kit_name(name):
        raise ValueError(f"nome de kit inválido para caminho: {name!r} (use letras, dígitos, '.', '_' ou '-')")
    return name
    ...
    if skill_root.is_dir():
        for skill_dir in sorted(p for p in skill_root.iterdir() if p.is_dir() and (p / "SKILL.md").is_file()):
            ...
            if dest_name in slug_owner:
                ...
                return {
                    "status": "error",
                    "detail": f"colisão de slug: skills {slug_owner[dest_name]!r} e {skill_dir.name!r} "
                              f"geram o mesmo destino {dest_name}",
                }, 3
...
def _self_test() -> int:
    ...
    try:
        ...
        print("self-test OK — plano não escreve; runtime e skill namespaced copiados; rerun idempotente; "
              "nome com travessia e colisão de slug são erro 3")
        return 0
    finally:
