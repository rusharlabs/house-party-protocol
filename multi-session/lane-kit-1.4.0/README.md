...
In the emitted distribution this module lives in `multi-session/lane-kit-1.4.0/` (the
...
```bash
KIT=multi-session/lane-kit-1.4.0
...
```bash
python lane-kit/scripts/lane_board.py compete --task TASK-1 --items ITEM-A,ITEM-B --lane coord --model claude-opus-4-8
python lane-kit/scripts/lane_board.py select --task TASK-1 --winner ITEM-A --lane rev-x --model gpt-5.6 --reason "same tests, half the diff"
python lane-kit/scripts/lane_board.py select --task TASK-1 --checker-unavailable --lane rev-x --model gpt-5.6
```
...
| `select` needs every candidate CHECKPOINT-READY with evidence, or VERIFIED | comparing against an unfinished attempt is not a comparison |
...
cannot drift on to MERGED; each losing lane is owed the news in `effects.json` and shows under
**UNDELIVERED** until someone tells it. The winner keeps its own state and still goes through its
ordinary review — a selection compares attempts, it does not verify one. `render` prints a
**COMPETITIONS** section with each task's outcome.
