...
- **select**: every candidate CHECKPOINT-READY with evidence (or VERIFIED); the reviewer's lane differs from every candidate's builder lanes and its model family from every builder's family; the winner is one of the candidates. The losers become `NOT-SELECTED` and their lanes are owed the news (`lane_effects.py pending`).
