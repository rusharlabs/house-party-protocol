...
Na distribuição emitida este módulo vive em `multi-session/lane-kit-1.4.0/` (o diretório
...
| o `select` exige todo candidato em CHECKPOINT-READY com evidência, ou VERIFIED | comparar contra uma tentativa inacabada não é comparação |
...
`effects.json` e aparece sob **UNDELIVERED** até alguém avisá-la. O vencedor mantém o próprio estado
e ainda passa pela revisão comum — uma seleção compara tentativas, não verifica uma. O `render`
imprime uma seção **COMPETITIONS** com o desfecho de cada tarefa.
