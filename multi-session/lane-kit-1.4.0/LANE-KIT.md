...
for the same task, and a reviewer keeps the best attempt. The board records it with two commands
(`compete`, `select`) and one extra item state (`NOT-SELECTED`):
...
1. **`compete --task T --items A,B[,C...]`** declares the items as candidates of `T`. Each must
   ...
   once.
2. **`select --task T --winner A`** needs every candidate `CHECKPOINT-READY` with evidence, or
   `VERIFIED`, and a reviewer whose lane differs from EVERY builder lane of EVERY candidate and
   whose model family differs from every builder's family. The competition event keeps the winner,
   the losers, the reviewer, the reason and the evidence it compared.
