...
item para a mesma tarefa, e uma revisora fica com a melhor tentativa. O board registra isso com dois
comandos (`compete`, `select`) e um estado de item a mais (`NOT-SELECTED`):
...
2. **`select --task T --winner A`** exige todo candidato em `CHECKPOINT-READY` com evidência, ou
   `VERIFIED`, e uma revisora cuja lane difere de TODAS as lanes construtoras de TODOS os
   candidatos e cuja família de modelo difere de todas as famílias construtoras. O evento da
   competição guarda o vencedor, os perdedores, a revisora, o motivo e a evidência comparada.
