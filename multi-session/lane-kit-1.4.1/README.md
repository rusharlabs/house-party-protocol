...
In the emitted distribution this module lives in `multi-session/lane-kit-1.4.1/` (the
...
```bash
KIT=multi-session/lane-kit-1.4.1
...
python lane-kit/scripts/lane_board.py withdraw --task TASK-1 --item ITEM-C --lane coord --model claude-opus-4-8 --reason "lane exec-c died mid-build"
```
