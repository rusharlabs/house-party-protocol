...
def register(lane_id: str, role: str, session_id: str, model: str,
             branch: str = "", territory: dict | None = None, worktree: str = "",
             evicted_out: list | None = None) -> tuple:
    """Register this session as a live lane. `evicted_out` collects the dead lanes dropped here.

    Why `worktree` and `evicted_out` exist (lane_rescue.py): a lane IS a worktree, and dropping
    its registry entry is the moment it becomes unowned — from then on any cleanup can delete
    uncommitted work with no trace. The caller needs to know WHICH lanes were dropped and WHERE
    their worktrees are, so it can take a rescue patch before anything touches the disk. The
    capture itself is not done here: it is git I/O and would hold the registry lock.
    """
    cfg = load_config()
    try:
            ...
            for lid in list(lanes):
                if liveness(lanes[lid], cfg) == "dead":
                    if evicted_out is not None:
                        evicted_out.append((lid, dict(lanes[lid])))
                    del lanes[lid]
        ...
        return False, f"lane_register: lock not acquired within 2s — registration skipped (WARN, the hook never hangs): {e}"
...
def _self_test() -> int:
    ...
    try:
        ...
        assert set(reg["lanes"]["exec-a"].keys()) == {
            "role", "session_id", "model", "branch", "started_at", "heartbeat_at", "territory",
            "worktree", "status"
        ...
        print("self-test OK — register creates/evicts dead lanes/keeps started_at, heartbeat throttle+advance, "
              "liveness alive/suspect/dead, who_owns exclusive+glob**+ignores dead, alive_others excludes self, "
              "lock contention fails fast without hanging, corrupt registry degrades cleanly")
        return 0
    finally:
