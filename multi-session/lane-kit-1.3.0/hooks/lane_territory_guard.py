#!/usr/bin/env python3
"""
lane_territory_guard -- PreToolUse (Edit|Write): warns about territory collisions between lanes.

WARN-only (house doctrine: never emits `decision`, never blocks, always exit 0) -- same
...
Usage (hook): echo '{"tool_input":{"file_path":"/abs/path.py"}}' | python lane_territory_guard.py
Exit: always 0.
...
def handle(payload: dict) -> str | None:
    """Returns the warning text (or None). Never raises, never signals decision/block."""
    ...
    if red_hit:
        warnings.append(
            f"[lane_territory_guard] WARNING: {rel} is a RED ZONE ({red_hit}) — "
            f"no lane edits it without the operator gate. WARN-only — not blocking."
        )
    ...
    if owners:
        ...
        warnings.append(
            f"[lane_territory_guard] WARNING: {rel} is in the EXCLUSIVE territory of lane {lid} "
            f"(heartbeat {age}, {state}) — coordinate via lane_board / REORIENT-MAILBOX before editing. WARN-only."
        )
...
def _self_test() -> int:
    ...
    try:
        ...
        assert w1 and "exec-a" in w1, f"should warn about exec-a territory: {w1}"

        # 2. path outside the territory -> silence
        p2 = str(tmp / "scripts" / "other" / "y.py")
        w2 = handle({"tool_input": {"file_path": p2}})
        assert w2 is None, f"outside the territory it should not warn: {w2}"

        # 3. exec-a editing its own territory -> silence
        os.environ["CLAUDE_LANE_ID"] = "exec-a"
        w3 = handle({"tool_input": {"file_path": p1}})
        assert w3 is None, f"the owner editing its own territory should not warn: {w3}"
        os.environ["CLAUDE_LANE_ID"] = "exec-b"

        # 4. exec-a with a 35min heartbeat (dead) -> zero false positive
        ...
        assert w4 is None, f"a dead lane should not trigger territory: {w4}"

        # 5. red zone with an empty registry -> warns even solo
        ...
        assert w5 and "RED ZONE" in w5, f"a red zone should warn even solo: {w5}"

        # 6. red zone glob ** (nested MEMORY.md)
        p6 = str(tmp / "deep" / "nested" / "MEMORY.md")
        w6 = handle({"tool_input": {"file_path": p6}})
        assert w6 and "RED ZONE" in w6, f"** should match a nested MEMORY.md: {w6}"

        # 7. payload without file_path / garbage -> None, exit 0 via main()
        assert handle({"tool_input": {}}) is None
        assert handle({}) is None

        # 8. main()'s result never contains "decision" (structural WARN-only)
        import io as _io
        old_stdin = sys.stdin
        ...
        print("self-test OK — exclusive territory names the right owner, outside the territory is silent, "
              "the owner in its own territory is silent, dead lane = zero false positives, red zone "
              "warns even solo, glob ** works, empty payload = None, main() always exits 0")
        return 0
    finally:
