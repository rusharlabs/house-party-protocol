...
def route(maker: str, prefer: tuple[str, ...]) -> dict:
    ...
    return {
        ...
        "available": available,
        "note": "the router only selects; permissions and the read-only prompt stay explicit in the caller",
    }
...
def _self_test() -> int:
    ...
    print("self-test OK — detects Codex/Cursor/Gemini and never picks the maker as checker")
    return 0
