...
By design: ALWAYS exit 0. The decision travels via JSON (`decision`/`permissionDecision`),
...
def handle(payload: dict) -> dict:
    ...
    if os.environ.get("LANE_GIT_GUARD_BYPASS") == "1":
        sys.stderr.write("[lane_git_guard] bypass active (LANE_GIT_GUARD_BYPASS=1) — not evaluating.\n")
        return {}
    ...
    if not others:
        ...
        if heuristic_on and age is not None and age < heuristic_minutes:
            msg = (
                f"[lane_git_guard] WARNING (heuristic, no registry.json): last commit was "
                f"{age:.0f}min ago — another session may be active without having registered a lane yet. "
    ...
    msg = (
        f"⛔ LANE-GIT-GUARD: {len(others)} live lane(s) ({others_str}). The git index is SHARED "
        f"between lanes — use `git commit -m '...' -- <paths>` (never -a / add -A / commit without a pathspec). "
        f"Families detected: {', '.join(hits)}. Command: {command[:120]!r}"
    )
...
def _self_test() -> int:
    ...
    for name, cmd in positives.items():
        assert detect(cmd), f"should detect a dangerous family in: {name!r} -> {cmd!r}"
    ...
    for cmd in negatives:
        assert not detect(cmd), f"should NOT detect in: {cmd!r} -> {detect(cmd)}"

    # extra_patterns via config
    hits_extra = detect("git restore .", {"git_guard": {"extra_patterns": [r"git\s+restore\s+\."]}})
    assert hits_extra, "extra_patterns should catch git restore ."
    ...
    try:
        ...
        assert "decision" not in out_warn, f"warn mode should have no decision: {out_warn}"
        ...
        assert out_dead == {}, f"a dead lane should trigger nothing: {out_dead}"

        print("self-test OK — 13 positives + 9 negatives in detect(), extra_patterns works, "
              "solo releases, warn does not block, block blocks, explicit pathspec releases, bypass releases, "
              "suspect-only forces warn, dead lane = zero false positives")
        return 0
    finally:
        os.environ.pop("LANE_GIT_GUARD_MODE", None)
        os.environ.pop("LANE_GIT_GUARD_BYPASS", None)
