...
# UX-INSTALL-JOURNEY — a jornada canônica de instalação de um módulo, contada na conversa
...
> (o molde do README por-módulo); os dois viajam na raiz da distribuição emitida. Este documento
> descreve a EXPERIÊNCIA: como um AGENTE (Claude Code) guia um HUMANO pela instalação, numa
> conversa.
> Nada aqui muda a mecânica — se este documento contradisser `INSTALL-CONTRACT.md`, o contrato
> vence.
...
então re-invoca com `--apply`. Não existe `input()` em lugar nenhum — o "Confirm" é a dupla
invocação.
...
integridade com `kit_doctor.py verify <kit>`, ou baixar o módulo de novo). Só volta a oferecer
aplicação depois de um plano novo sair limpo. Um humano que insiste em aplicar com smoke falhando
está por conta própria — o agente registra que desaconselhou e por quê.
...
O texto que o agente cola na conversa ao apresentar um plano. Placeholders em `{}`. As linhas
marcadas `[cenário]` só entram no cenário correspondente. O molde está escrito em inglês; o
agente fala na língua do humano e adapta as palavras, não a estrutura.
...
- [ ] NUNCA edita `settings.local.json` / hooks — apresenta o bloco, o humano cola
