...
| HPP core (`python -m hpp`) | any with `hpp decide` | no — only for the offline relabel of `unknown` (skill step 5) |
...
In the emitted distribution this module lives in `continuity/gotcha-memory-1.0.3/` (the
...
- `unknown` is never auto-relabelled; an advisory relabel via `hpp decide` is offline, run by a person (`--export-unknown`, step 5 of the skill; requires the HPP core). A new family signal is a code change, never a record.

## Seed and licence

The module ships no built-in lessons. Seed your own with `seed_from_file()`, using
`curated.seed.example.yaml` as the template. MIT licence (see `LICENSE`).
