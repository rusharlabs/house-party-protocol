...
Written from scratch out of failures observed in real sessions and documented
as rules -- each family below records why it exists. It is the taxonomy of
whoever operates Claude Code, built from that operator's own failures.
...
and the consumer's contract requires exactly these names:
  ...
  unknown      nothing matched -- and the default is RETRY, because an unknown error is rarely
               ...
               It is never relabelled automatically: an advisory relabel is OFFLINE and run by
               a person (`gotchas_memory.py --export-unknown` + `hpp decide eval`, step 5 of
               skills/gotcha-memory/SKILL.md); a new signal enters this table only as a code change.
...
_FAMILIES: tuple = (
    ...
    Family(
        ...
        strategy=RETRY,
        why=(
            "an index.lock left behind in a shared repo: this is not "
    ...
    Family(
        ...
        strategy=ESCALATE,
        why=(
            "an empty secret can be written with exit 0. Bad "
