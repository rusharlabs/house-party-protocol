...
In the emitted distribution this module lives in `continuity/continuity-kit-1.4.0/` (the
directory carries the version — state it once, in `KIT`). The installer is
`installers/kit-forge-1.4.2/kit_doctor.py` — the single installation engine of the whole
...
```bash
KIT=continuity/continuity-kit-1.4.0
