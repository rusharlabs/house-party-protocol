...
after a significant session, WITHOUT letting them grow into logs nobody reads.

Maintains a set of history documents (changelog + narrative timeline + state
snapshot + lessons + per-session wrapup) declared in a `rollup.yaml` config file,
so the same script serves any project without code changes.
...
stdlib + PyYAML.
v1.0.0 -- 2026-07-10 (continuity-kit - generic doc-rollup)
"""
