...
Portable module of the distributable `gotcha-memory` kit.
Design notes:

  1. The `error_strategy` import is from the SIBLING `_lib/` (vendored), so the kit needs no external package.
  ...
  3. No hardcoded guardrail seed ships with the kit (a seed is personal
     content -- it does not belong in a distributable kit). Instead,
...
v1.0.0 -- 2026-07-11 (gotcha-memory kit -- standalone module)
"""
...
from error_strategy import FAMILIES, select_strategy  # noqa: E402  (vendored sibling)
...
# ─────────────────────────── offline relabel of `unknown` (human-run) ───────────────────────────
# The question matches the HPP core's gotcha-family example suite, so a decider measured there
# answered the same question. `unknown` is not an option: an abstention maps back to it.
_RELABEL_QUESTION: dict[str, Any] = {
    "id": "gotcha.family",
    "kind": "choice",
    "options": [f for f in FAMILIES if f != "unknown"],
    "ladder": False,
    "instructions": "A shell command failed. Which family of failure does its error output belong to? "
                    "Answer from the error text only.",
    "criteria": {
        "instrument": "the measuring tool itself misbehaved (pipe closed early, truncated stream, unsorted input to a sorting tool)",
        "lock": "another process holds a lock or the resource is busy",
        "ratelimit": "a service refused because of rate limit or quota",
        "transient": "a network or service hiccup that may succeed on retry",
        "state": "the repository or filesystem is in an unexpected state",
        "config": "the command, its options, its permissions or its environment are wrong",
        "dependency": "a required package or module is missing or cannot be resolved",
        "fatal": "the process crashed or was killed by the operating system",
    },
}
# Why: `hpp decide` refuses a state that looks like it carries a secret, and one such case aborts the
# whole suite. A message the redactor already touched, or one that keeps a `key=`/`key:` shape, stays
# out of the export: counted, never written.
_EXPORT_REFUSE = re.compile(
    r"\[REDACTED:|(?:api[_-]?key|access[_-]?key|secret|password|passwd|token|private[_-]?key)[\"']?\s*[:=]"
    r"|-----BEGIN [A-Z ]+-----|\bsk-[A-Za-z0-9_\-]{8,}",
    re.IGNORECASE,
)


def export_unknown_suite(out_path: str | Path, *, store_dir: str | Path | None = None) -> dict[str, Any]:
    """Writes the store's `unknown` messages as an `hpp.decision-suite/v1` for an OFFLINE, human-run relabel.

    Reads failures.jsonl and failures.archive.jsonl, keeps family `unknown`, re-applies redaction,
    leaves secret-shaped messages out and merges repeats (`count`). Every case is written with
    `expected: null`: a person labels it (a family, or `abstain`) before `hpp decide eval`, which
    refuses an unlabelled case. Never overwrites `out_path` (it may hold those labels). No hook calls
    this and nothing in the kit reads a decision. Returns {status: written|empty|refused, ...}.
    """
    out = Path(out_path)
    store = _store(store_dir)
    result: dict[str, Any] = {"status": "empty", "path": str(out), "store": str(store),
                              "cases": 0, "skipped_secret": 0, "skipped_blank": 0, "merged": 0}
    if out.exists():
        result["status"] = "refused"
        return result
    cases: dict[str, dict[str, Any]] = {}
    for name in ("failures.archive.jsonl", "failures.jsonl"):
        for event in _read_jsonl(store / name):
            if event.get("family") != "unknown":
                continue
            text = redact_secrets(str(event.get("error") or "")).strip()
            if not text:
                result["skipped_blank"] += 1
            elif _EXPORT_REFUSE.search(text):
                ...
                cases[text] = {"id": f"unknown-{digest}", "stratum": "store-unknown", "expected": None,
                               "count": 1, "state": text}
    if not cases:
        return result
    suite = {
        "schema": "hpp.decision-suite/v1",
        "name": "gotcha-unknown-relabel",
        "note": ("Exported from a gotcha-memory store: messages the classifier left as `unknown`, redacted, "
                 "secret-shaped ones left out. Label every case's `expected` with a family, or `abstain` when the "
                 "text is genuinely ambiguous (abstain maps back to `unknown`), before `python -m hpp decide eval`; "
                 "an unlabelled case is refused. Advisory only: no hook reads a decision."),
        "question": _RELABEL_QUESTION,
    ...
    try:
        with out.open("x", encoding="utf-8") as f:
            f.write(json.dumps(suite, ensure_ascii=False, indent=2) + "\n")
    except FileExistsError:
        result["status"] = "refused"
        return result
    result.update(status="written", cases=len(cases))
    return result
...
def _self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        # Offline relabel export: only `unknown`, redacted, merged, secret-shaped left out, never overwritten
        rel = store / "relabel"
        record_failure("task:odd", "weird thing happened", store_dir=rel, now=t0)
        record_failure("task:odd", "weird thing happened", store_dir=rel, now=t0 + 1)
        record_failure("task:odd", "ETIMEDOUT", store_dir=rel, now=t0 + 2)
        record_failure("task:odd", "odd refusal password=hunter2hunter2", store_dir=rel, now=t0 + 3)
        suite_path = store / "unknown-suite.json"
        res = export_unknown_suite(suite_path, store_dir=rel)
        assert res["status"] == "written" and res["cases"] == 1 and res["skipped_secret"] == 1, res
        suite = json.loads(suite_path.read_text(encoding="utf-8"))
        assert suite["schema"] == "hpp.decision-suite/v1", suite
        opts = suite["question"]["options"]
        assert "unknown" not in opts and "abstain" not in opts and len(opts) == len(FAMILIES) - 1, opts
        (case,) = suite["cases"]
        assert case["state"] == "weird thing happened" and case["count"] == 2 and case["expected"] is None, case
        assert "hunter2" not in suite_path.read_text(encoding="utf-8"), "secret reached the export"
        before = suite_path.read_bytes()
        assert export_unknown_suite(suite_path, store_dir=rel)["status"] == "refused", "must not overwrite labels"
        assert suite_path.read_bytes() == before, "a refused export changed the labelled file"
        # CONTROL: a store without `unknown` writes nothing (this one holds transient + config only)
        empty = export_unknown_suite(store / "none.json", store_dir=store)
        assert empty["status"] == "empty" and not (store / "none.json").exists(), empty
        print("self-test OK ✓")
...
if __name__ == "__main__":
    ...
    elif arg == "--export-unknown" and len(sys.argv) > 2:
        r = export_unknown_suite(sys.argv[2])
        left_out = f"{r['skipped_secret']} secret-shaped and {r['skipped_blank']} blank left out"
        if r["status"] == "refused":
            print(f"refused: {r['path']} already exists (it may hold your labels); nothing written", file=sys.stderr)
            sys.exit(2)
        elif r["status"] == "empty":
            print(f"Nothing to export: no `unknown` message in {r['store']} ({left_out}); no file written")
        else:
            print(f"Exported {r['cases']} unknown message(s) to {r['path']} ({left_out}, {r['merged']} repeats merged). "
                  "Label each case's expected (a family, or abstain) before `python -m hpp decide eval`.")
    else:
        _demo()
