...
| Núcleo HPP (`python -m hpp`) | qualquer com `hpp decide` | não — só p/ a reclassificação offline de `unknown` (passo 5 da skill) |
...
├── _lib/
│   ├── gotchas_memory.py         ← core: record/recurring/curated/preamble + CLI (--seed/--preamble/--export-unknown/--self-test/demo)
...
Na distribuição emitida este módulo vive em `continuity/gotcha-memory-1.0.2/` (o
...
```bash
KIT=continuity/gotcha-memory-1.0.2
...
python _lib/gotchas_memory.py --export-unknown relabel.json    # unknown messages -> a suite to label (offline, skill step 5)
```
...
- `unknown` nunca é reclassificado automaticamente; uma reclassificação consultiva via `hpp decide` é offline, rodada por uma pessoa (`--export-unknown`, passo 5 da skill; requer o núcleo HPP). Sinal novo de família é mudança de código, nunca um registro.

## Seed e licença

O módulo não traz lições embutidas: as regras curadas entram por `seed_from_file()` a partir de um YAML seu, com `curated.seed.example.yaml` como ponto de partida genérico. Licença MIT (ver `LICENSE`).
