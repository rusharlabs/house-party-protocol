#!/usr/bin/env python3
"""
session_boot -- generic SessionStart (no agent persona/identity).

Builds the boot context: git HEAD + branch, the live sections of the state-doc (config-driven,
not hardcoded, capped at `MAX_SECTION=3500` per section), and the newest handoff-v1 read through
`_handoff_io` -- `handoff_guard.py` writes the handoff automatically and this hook reads it back
automatically, so both ends are covered. Fail-open throughout.
...
stdlib only. v1.0.0 -- 2026-07-10 (continuity-kit)
"""
...
MAX_SECTION = 3500  # cap per section, not the total
...
def main(argv) -> int:
    ...
    except Exception:  # noqa: BLE001 -- total fail-open: a broken boot hook must never block the session
        pass
    return 0
