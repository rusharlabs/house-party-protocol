...
v1.0.0 -- 2026-07-10 (continuity-kit - Tier 1)
"""
...
# An `hpp evidence` record as `already_done[].evidence` (the path `python -m hpp evidence run` writes).
# render only names it: re-checking is `python -m hpp evidence verify`, never a hash taken here.
# Why: a record is known by the basename the core gives it, `<id>-<YYYYMMDDTHHMMSSZ>[-N].json` — the
# directory can be any `--out`, and on Windows the path arrives with backslashes. The whole value must
# still be one path (no whitespace or `;`), so prose that merely mentions a record is not named as one.
_HPP_RECORD_NAME = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,79}-\d{8}T\d{6}Z(?:-\d+)?\.json$")


def _is_hpp_record(evidence: str) -> bool:
    path = evidence.replace("\\", "/")
    return bool(path) and not re.search(r"[\s;]", path) and bool(_HPP_RECORD_NAME.match(path.rsplit("/", 1)[-1]))
...
def render(h: dict, cap_bytes: int = 4096) -> str:
    ...
    if stale:
        ...
        if ja:
            ...
            records = [str(j["evidence"]) for j in ja if _is_hpp_record(str(j.get("evidence") or ""))]
            if records:
                lines.append("  evidence records (python -m hpp evidence verify <record> -> exit 0 = intact passed run): "
                             + "; ".join(records))
...
def _self_test() -> int:
    ...
    try:
        ...
        # CONTROL: plain evidence renders exactly as before — no evidence-record line.
        plain = dict(good, already_done=[{"action": "wrote the notes", "evidence": "notes.md"}])
        text_plain = render(plain)
        assert "ALREADY EXECUTED — NEVER REPEAT: wrote the notes\n" in text_plain, f"already_done line changed: {text_plain}"
        assert "hpp evidence" not in text_plain, f"plain evidence must not be named as a record: {text_plain}"
        # An hpp evidence record is named, with the command that re-checks it (render never re-hashes).
        rec_path = ".hpp/evidence/e2e-login-20260924T120000Z.json"
        with_rec = dict(good, already_done=[{"action": "ran the e2e spec", "evidence": rec_path},
                                            {"action": "wrote the notes", "evidence": "notes.md"}])
        rec_lines = [ln for ln in render(with_rec).splitlines() if "python -m hpp evidence verify" in ln]
        assert len(rec_lines) == 1 and rec_path in rec_lines[0] and "notes.md" not in rec_lines[0], \
            f"an hpp evidence record should be named on one line: {rec_lines}"
        # A record is known by its basename `<id>-<YYYYMMDDTHHMMSSZ>[-N].json`, wherever it was written:
        # a Windows path, a custom --out directory, the collision suffix the core adds.
        for other in (".hpp\\evidence\\e2e-login-20260924T120000Z.json",
                      "C:\\work\\repo\\.hpp\\evidence\\e2e-login-20260924T120000Z.json",
                      "qa-scratch/records/e2e-login-20260924T120000Z.json",
                      ".hpp/evidence/e2e-login-20260924T120000Z-2.json"):
            named = [ln for ln in render(dict(good, already_done=[{"action": "ran it", "evidence": other}])).splitlines()
                     if "python -m hpp evidence verify" in ln]
            assert len(named) == 1 and other in named[0], f"{other!r} should be named as a record: {named}"
        # CONTROL: a json that is not named like a record, and prose that merely ends in one, are not records.
        for not_record in (".hpp/evidence/notes.json", "see qa/e2e-login-20260924T120000Z.json then reran",
                           "report.json"):
            assert "hpp evidence" not in render(dict(good, already_done=[{"action": "x", "evidence": not_record}])), \
                f"{not_record!r} must not be named as a record"
