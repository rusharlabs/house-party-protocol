...
Fail-open: any error = exit 0 with no output, never brings
down the session boot. Transition compat: no JSON handoff but with RESUME-NEXT.md (the pre-JSON
...
def main(argv) -> int:
    ...
    except Exception:  # noqa: BLE001 -- total fail-open: never brings down the session boot
        pass
    return 0
