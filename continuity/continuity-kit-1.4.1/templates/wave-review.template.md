...
```bash
python {{lane_board}} set {{item_id}} CHECKPOINT-READY \
...
```bash
python {{lane_board}} set {{item_id}} {{VERIFIED_or_NEEDS_FIX_or_DEFERRED}} \
...
```bash
python {{lane_board}} select --task {{task_id}} --winner {{item_id}} \
