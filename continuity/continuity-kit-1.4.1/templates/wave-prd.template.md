...
{{criterion_command_2}}
{{criterion_command_3}}
# e2e/visual: record it (requires the HPP core); the exit code is the verdict, the trace is the proof.
# Double quotes keep the glob literal in bash AND cmd.exe; a per-run output dir keeps the trace verifiable.
# python -m hpp evidence run --id {{item}}-e2e --artifact "{{e2e_out_dir}}/**/trace.zip" -- npx playwright test {{spec}}
```
