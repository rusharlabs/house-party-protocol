...
| R2 | Gate | the PRD's DoD passes LIVE | `done_gate.py` exit 0 (probes), or a passed, verified `hpp evidence` record (`python -m hpp evidence verify <record>` exit 0; requires the HPP core) | only the script (never prose) |
...
only by a reviewer from ANOTHER lane AND ANOTHER model family (maker≠checker refused with exit 1, not
