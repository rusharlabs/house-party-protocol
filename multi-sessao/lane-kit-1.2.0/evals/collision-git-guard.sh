...
run_round() {
  ...
  if ! echo "$OUT" | grep -q '"decision"' && echo "$ERR" | grep -q "LANE-GIT-GUARD" && echo "$ERR" | grep -q "exec-a"; then
    report 0 "round$round G1a: modo warn avisa (stderr cita exec-a), sem decision"
  else
    report 1 "round$round G1a falhou: OUT=$OUT ERR=$ERR"
  fi
  ...
  echo "$OUT" | grep -q '"decision": *"block"' && echo "$OUT" | grep -q '"permissionDecision": *"deny"' \
    && report 0 "round$round G1b: modo block -> decision:block + permissionDecision:deny" \
    || report 1 "round$round G1b falhou: OUT=$OUT"

  # G2a — comando seguro (pathspec explicito) sempre libera, mesmo em modo block
  OUT=$(invoke_guard "git commit -m x -- file.py")
  [ "$OUT" = "{}" ] && report 0 "round$round G2a: pathspec explicito libera mesmo em modo block" \
    || report 1 "round$round G2a falhou: OUT=$OUT"

  # G2b — solo (evict exec-a) libera mesmo com -am
  python "$LANE_IO" evict --lane exec-a >/dev/null
  OUT=$(invoke_guard "git commit -am x")
  [ "$OUT" = "{}" ] && report 0 "round$round G2b: solo libera mesmo com -am" \
    || report 1 "round$round G2b falhou: OUT=$OUT"
...
"
  OUT=$(invoke_guard "git commit -am x")
  [ "$OUT" = "{}" ] && report 0 "round$round G2c: lane morta = zero falso-positivo" \
    || report 1 "round$round G2c falhou: OUT=$OUT"
...
try:
    ...
    print('FAIL:' + str(e))
")
  [ "$VALID" = "ok" ] && report 0 "round$round rider: 10 processos concorrentes -> registry.json continua JSON valido" \
    || report 1 "round$round rider falhou: $VALID"
...
echo ""
TOTAL=$((PASS + FAIL))
echo "collision-git-guard: $PASS/$TOTAL checks verdes em 3 rodadas (pass^k=1.00 exige $TOTAL/$TOTAL)"
