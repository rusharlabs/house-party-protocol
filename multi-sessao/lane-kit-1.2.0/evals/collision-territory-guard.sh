...
run_round() {
  ...
  if [ "$OUT" = "{}" ] && echo "$ERR" | grep -q "exec-a" && echo "$ERR" | grep -qE "[0-9]+min"; then
    report 0 "round$round T1a: territorio exclusivo avisa exec-a + idade, sem decision"
  else
    report 1 "round$round T1a falhou: OUT=$OUT ERR=$ERR"
  fi
  ...
  ERR=$(cat "$STDERR_TMP" 2>/dev/null)
  [ "$OUT" = "{}" ] && [ -z "$ERR" ] && report 0 "round$round T1b: fora do territorio = silencio" \
    || report 1 "round$round T1b falhou: OUT=$OUT ERR=$ERR"
  ...
  OUT=$(invoke_guard "$WORK_N/scripts/vm/x.py")
  ERR=$(cat "$STDERR_TMP" 2>/dev/null)
  [ "$OUT" = "{}" ] && [ -z "$ERR" ] && report 0 "round$round T1c: dono no proprio territorio = silencio" \
    || report 1 "round$round T1c falhou: OUT=$OUT ERR=$ERR"
  export CLAUDE_LANE_ID="exec-b"
  ...
  ERR=$(cat "$STDERR_TMP" 2>/dev/null)
  [ "$OUT" = "{}" ] && echo "$ERR" | grep -q "ZONA VERMELHA" && report 0 "round$round T2a: zona vermelha avisa (settings.local.json)" \
    || report 1 "round$round T2a falhou: OUT=$OUT ERR=$ERR"

  # T2b — glob ** casa MEMORY.md aninhado
  : > "$STDERR_TMP"
  OUT=$(invoke_guard "$WORK_N/deep/nested/MEMORY.md")
  ERR=$(cat "$STDERR_TMP" 2>/dev/null)
  [ "$OUT" = "{}" ] && echo "$ERR" | grep -q "ZONA VERMELHA" && report 0 "round$round T2b: glob ** casa MEMORY.md aninhado" \
    || report 1 "round$round T2b falhou: OUT=$OUT ERR=$ERR"
...
"
  ...
  if [ "$OUT" = "{}" ] && ! echo "$ERR" | grep -q "exec-a"; then
    report 0 "round$round T2c: lane morta reivindicando territorio = zero falso-positivo"
  else
    report 1 "round$round T2c falhou: OUT=$OUT ERR=$ERR"
  fi
...
echo ""
TOTAL=$((PASS + FAIL))
echo "collision-territory-guard: $PASS/$TOTAL checks verdes em 3 rodadas (pass^k=1.00 exige $TOTAL/$TOTAL)"
