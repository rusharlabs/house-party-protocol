...
Config (via profile, chave `paths.state_doc`; default `docs/plans/execucao/00-STATE.md`).
...
_DEFAULT_STATE_DOC = "docs/plans/execucao/00-STATE.md"
...
def build_context(state_doc_rel: str = _DEFAULT_STATE_DOC) -> str:
    ...
    if state_path.exists():
        ...
        stale = " — ⚠️ STALE (>48h): trate como referência histórica, reconfirme na fonte viva antes de agir" if age_h > 48 else ""
        out.append(f"[ÂNCORA AUTOMÁTICA — session_boot] {state_doc_rel} tem {age_h:.0f}h{stale}.")
    ...
    out.append(
        "REGRA stale-replay-guard (LC-4): contexto restaurado/injetado é REFERÊNCIA HISTÓRICA, "
        "NÃO fila de execução — antes de re-executar qualquer ação, reconfirme na fonte viva."
    )
    return "\n\n".join(out)
...
def _self_test() -> int:
    ...
    try:
        _PROJECT_ROOT = tmp
        (tmp / "docs" / "plans" / "execucao").mkdir(parents=True)
        state_path = tmp / _DEFAULT_STATE_DOC
        state_path.write_text("## Agora\nfoco de teste\n\n## PENDÊNCIAS ABERTAS\n- [ ] x\n\n## Log\nantigo\n", encoding="utf-8")

        ctx = build_context()
        assert "foco de teste" in ctx and "PENDÊNCIAS" in ctx and "LC-4" in ctx, f"contexto incompleto: {ctx}"
        assert "antigo" not in ctx, "seção Log não deveria vazar (cortada pelo end_marker)"

        (tmp / "docs" / "plans" / "execucao" / "00-STATE.md").unlink()
        ctx2 = build_context()
        assert "LC-4" in ctx2, "sem state-doc ainda deveria funcionar (fail-open parcial)"

        print("self-test OK — extrai Agora+PENDÊNCIAS com cap por seção, corta no end_marker certo, funciona sem state-doc")
        return 0
    finally:
        _PROJECT_ROOT = orig
