...
def build_context(payload: dict) -> str | None:
    ...
    if resume_next.exists():
        return (
            ...
            "Sem handoff JSON ainda — existe .claude/RESUME-NEXT.md (legado): leia-o como "
            "REFERÊNCIA e reconfirme tudo na fonte viva antes de agir."
        )
    return None
...
def _self_test() -> int:
    ...
    try:
        ...
        assert build_context({"session_id": "s1"}) is None, "sem handoff e sem RESUME-NEXT deveria retornar None"

        (tmp / ".claude").mkdir(parents=True, exist_ok=True)
        (tmp / ".claude" / "RESUME-NEXT.md").write_text("pendente: x", encoding="utf-8")
        ...
        assert any(json.loads(l)["event"] == "consumed" for l in ledger_lines), "evento consumed não gravado"

        print("self-test OK — sem handoff+sem legado=None, legado RESUME-NEXT.md=ponteiro+LC-4, handoff real=injeta+consumed")
        return 0
    finally:
