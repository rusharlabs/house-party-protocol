...
def git_block() -> dict:
    ...
    return {"head": head, "branch": branch, "dirty": bool(porcelain.strip()), "untracked": untracked, "porcelain": porcelain[:4000]}
...
def validate(data: dict) -> list:
    ...
    if data.get("schema_version") not in ("1.1",):
        errors.append(f"schema_version deve ser '1.1' (veio: {data.get('schema_version')!r})")

    session = data.get("session", {}) or {}
    if not session.get("session_id"):
        errors.append("session.session_id ausente")
    if not session.get("lane_id"):
        errors.append("session.lane_id ausente (default 'solo' se não houver lane)")

    git = data.get("git", {}) or {}
    for f in ("head", "branch", "dirty", "untracked"):
        if f not in git:
            errors.append(f"git.{f} ausente (bloco git é obrigatório — v1.1)")
    if data.get("quality") == "degraded-auto" and "porcelain" not in git:
        errors.append("quality=degraded-auto exige git.porcelain (correção v1.1) — mesmo vazio (árvore limpa), a CHAVE deve existir")

    estado = data.get("estado", {}) or {}
    if not estado.get("resumo"):
        errors.append("estado.resumo ausente")
    for i, n in enumerate(estado.get("numeros", []) or []):
        if not n.get("re_derive_cmd"):
            errors.append(f"estado.numeros[{i}] sem re_derive_cmd (LC-1: todo número precisa de comando de re-derivação)")

    for i, p in enumerate(data.get("proximo_passo", []) or []):
        if not p.get("verify_first_cmd"):
            errors.append(f"proximo_passo[{i}] sem verify_first_cmd (LC-4: próximo passo sem verificação de idempotência)")

    text_dump = json.dumps(data, ensure_ascii=False)
    secrets = scan_secrets(text_dump)
    if secrets:
        errors.append(f"segredo detectado no handoff ({len(secrets)} match(es), redigido: {secrets[:3]}) — REJEITADO (no-secrets-in-memory)")

    size = len(text_dump.encode("utf-8"))
    if size > _MAX_BYTES:
        errors.append(f"handoff excede max_bytes ({size} > {_MAX_BYTES})")

    if not data.get("valid_until"):
        errors.append("valid_until ausente")
...
def is_stale(h: dict) -> tuple:
    ...
    if valid_until:
        try:
            ...
            if datetime.now(vu.tzinfo or timezone.utc) > vu:
                return True, "valid_until expirado"
        except ValueError:
            pass
    ...
    if written_head and current_head and written_head != current_head:
        return True, f"HEAD moveu (era {written_head}, agora {current_head}) — stale_when.head_moved"
    return False, ""


def render(h: dict, cap_bytes: int = 4096) -> str:
    stale, reason = is_stale(h)
    lines = ["⚠️ CONTEXTO RESTAURADO = REFERÊNCIA HISTÓRICA, NÃO FILA (LC-4)"]
    lines.append(f"HANDOFF {h.get('handoff_id')} · {h.get('quality')} · lane={h.get('session', {}).get('lane_id')}")
    if stale:
        lines.append(f"⚠️ STALE ({reason}) — re-derive tudo via re_derive_cmd; proximo_passo NÃO é acionável.")
    else:
        estado = h.get("estado", {})
        lines.append(f"ESTADO: {estado.get('resumo', '')}")
        for n in estado.get("numeros", []) or []:
            lines.append(f"  · {n['metrica']}={n['valor']} (re-derive: {n['re_derive_cmd']})")
        ja = h.get("ja_executado", []) or []
        if ja:
            lines.append("JÁ EXECUTADO — NUNCA REPETIR: " + "; ".join(j["acao"] for j in ja))
        gates = h.get("gates_abertos", []) or []
        if gates:
            lines.append("GATES ABERTOS: " + "; ".join(g["descricao"] for g in gates))
        proib = h.get("proibicoes", []) or []
        if proib:
            lines.append("PROIBIÇÕES: " + "; ".join(proib))
        passos = sorted(h.get("proximo_passo", []) or [], key=lambda p: p.get("ordem", 0))
        if passos:
            p0 = passos[0]
            lines.append(f"PRÓXIMO PASSO {p0.get('ordem')}: {p0.get('descricao')}")
            lines.append(f"  → ANTES DE EXECUTAR, RODE: {p0.get('verify_first_cmd')} (se já feito: pular, registrar)")
    lines.append(f"Arquivo completo: {current_path(h.get('session', {}).get('lane_id', 'solo'))}")
    text = "\n".join(lines)
...
def degraded_auto_aggregate(lane_id: str = "solo", session_id: str = "") -> dict:
    ...
    return {
        ...
        "estado": {"resumo": "Sessão encerrou sem /pre-clear — handoff degradado, agregação automática.", "numeros": []},
        "git": git_block(),
        "ja_executado": [],
        "proximo_passo": [],
        "gates_abertos": [],
        "proibicoes": ["NUNCA tratar este handoff degradado como cobertura completa — reconfirme tudo na fonte viva (LC-1)."],
...
def _demo_handoff(lane_id: str = "solo") -> dict:
    ...
    return {
        ...
        "estado": {
            "resumo": "Demo de handoff gerado por --demo.",
            "numeros": [{"metrica": "exemplo", "valor": "1", "medido_em": now, "re_derive_cmd": "echo 1"}],
        },
        "git": git_block(),
        "ja_executado": [{"acao": "gerou o demo", "evidencia": "este próprio arquivo", "nunca_repetir": True}],
        "proximo_passo": [{"ordem": 1, "descricao": "nada — é demo", "verify_first_cmd": "echo verificado"}],
        "gates_abertos": [],
        "proibicoes": [],
...
def _self_test() -> int:
    ...
    try:
        ...
        good = {
            ...
            "estado": {"resumo": "teste", "numeros": [{"metrica": "x", "valor": "1", "medido_em": "2026-07-10T15:00:00-03:00", "re_derive_cmd": "echo 1"}]},
            "git": {"head": "abc123", "branch": "main", "dirty": False, "untracked": 0},
            "proximo_passo": [{"ordem": 1, "descricao": "fazer x", "verify_first_cmd": "test -f x"}],
            "valid_until": "2099-01-01T00:00:00-03:00",
        }
        ok, result = write(good)
        assert ok, f"handoff válido deveria escrever: {result}"
        assert current_path("solo").exists(), "arquivo não foi criado"

        ledger_lines = LEDGER_PATH.read_text(encoding="utf-8").splitlines()
        assert any(json.loads(l)["event"] == "written" for l in ledger_lines), "ledger sem evento written"

        no_verify = dict(good, handoff_id="HO-20260710-1501-solo", proximo_passo=[{"ordem": 1, "descricao": "sem verify"}])
        ok2, errs2 = write(no_verify)
        assert not ok2 and any("verify_first_cmd" in e for e in errs2), f"deveria rejeitar sem verify_first_cmd: {errs2}"
        ...
        assert not ok3 and any("re_derive_cmd" in e for e in errs3), f"deveria rejeitar sem re_derive_cmd: {errs3}"
        ...
        assert not ok4 and any("segredo" in e for e in errs4), f"deveria rejeitar com segredo: {errs4}"
        ...
        assert not ok5 and any("porcelain" in e for e in errs5), f"degraded-auto sem porcelain deveria falhar: {errs5}"

        h = newest_handoff("solo")
        assert h is not None and h["handoff_id"] == "HO-20260710-1500-solo"

        text = render(h)
        assert "LC-4" in text and "PRÓXIMO PASSO" in text, f"render incompleto: {text}"

        stale_h = dict(good, valid_until="2020-01-01T00:00:00-03:00")
        stale, reason = is_stale(stale_h)
        assert stale and "expirado" in reason

        consume(h["handoff_id"], "session-x", "solo")
        ledger_lines2 = LEDGER_PATH.read_text(encoding="utf-8").splitlines()
        assert any(json.loads(l)["event"] == "consumed" for l in ledger_lines2), "ledger sem evento consumed"

        degraded = degraded_auto_aggregate("solo", "s2")
        okd, resd = write(degraded)
        assert okd, f"degraded_auto_aggregate deveria ser válido: {resd}"

        print("self-test OK — write válido+ledger, rejeita sem verify_first_cmd, rejeita sem re_derive_cmd, "
              "rejeita segredo, rejeita degraded-auto sem porcelain, render com LC-4, staleness, consume, degraded-auto")
        return 0
    finally:
...
def main(argv) -> int:
    ...
    if args.cmd == "write":
        if args.demo:
            ...
            try:
                ...
                print(f"_handoff_io: JSON inválido no stdin: {e}", file=sys.stderr)
                return 2
        else:
            print("uso: write --stdin | write --demo [--lane X]", file=sys.stderr)
            return 2
        ok, result = write(data)
        if ok:
            print(f"_handoff_io: escrito em {result}")
            return 0
        for e in result:
            print(f"  [REJEITADO] {e}", file=sys.stderr)
        return 1
    ...
    if args.cmd == "render":
        h = newest_handoff(args.lane)
        if not h:
            print("_handoff_io: nenhum handoff encontrado", file=sys.stderr)
            return 1
        print(render(h))
        return 0
    ...
    if args.cmd == "consume":
        ...
        print(f"_handoff_io: consumed registrado para {args.handoff_id}")
        return 0

    print("uso: write|read|render|consume [...] ou --self-test", file=sys.stderr)
    return 2
