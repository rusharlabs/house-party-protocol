...
> accepted as legacy) · **Origin:** distilled
> from the calibration ruler of the 4 best skills this team has built (`source-sync`,
> `github-workflow`, `verify-6-levels` and a fourth one for semantic search) plus clause C3
> (real execution), which not even those have.
> **Enforcement:** `skill_lint.py` — `instaladores/kit-forge/tools/skill_lint.py` at the
...
- **STATE IT TOUCHES** (legacy ESTADO QUE TOCA) — table `file → reads/writes → purpose`
  (the pattern of `.claude/skills/source-sync/SKILL.md:166-176`).
...
pattern of the house's good scripts (`--self-test`). `--run-proofs` executes the first line of the
