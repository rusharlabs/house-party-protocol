...
Why it never clobbers: a statusLine wiring that overwrites one already configured by
another tool, without asking, silently destroys the user's setup. This one NEVER does
that (see `apply_spec`).
