...
> são aceitos como legado).
> **Enforcement:** `skill_lint.py` — `installers/kit-forge/tools/skill_lint.py` na raiz do
...
- **STATE IT TOUCHES** (legado ESTADO QUE TOCA) — tabela `arquivo → lê/escreve → propósito`
  (uma linha por arquivo que a skill lê ou escreve).
...
rede, que demonstra o contrato e termina com exit 0 esperado. Herda o padrão `--self-test` de um
script bem-comportado. O `--run-proofs` executa a primeira linha do bloco a partir do diretório da
