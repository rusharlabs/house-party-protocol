...
**An `hpp evidence` record is not a review** — it is self-hashed, not signed. The checker runs
`verify`, then re-runs `record.command` with `python -m hpp evidence run` from its own checkout or
lane (`--out` stays inside the tree it runs in); a re-run in the maker's tree voids the review above.
...
*Doctrine only — no adapter is shipped with this kit.*
...
2. MAKER runs the tests via `hpp evidence run`; the checker gets the record path, runs `verify`,
