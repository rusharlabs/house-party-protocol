...
> **Origin:** Extension of `learned-corrections.md` (LC-1/LC-2/LC-3) — registered as **LC-4**. Applies to any LLM that resumes context from a previous session/snapshot.
...
### LC-4b · A continuation is not a resend

A later turn in the same thread does NOT repeat the original prompt. It carries continuation
guidance only — what changed, what the last attempt produced, and an explicit `attempt`
number — and it says to resume from the CURRENT workspace state, not from the task as first
stated. Resending the original is LC-4 committed by the driver instead of the agent: the
agent re-plans for a world that has already moved, and redoes the part that already landed.
