...
never blocks: a guard hook must never break the tool call. Complements, does not duplicate, the commit gate.
...
def _warn(file_path: str, hits: list[str]) -> None:
    ...
    sys.stderr.write(
        ...
        "  -> Do NOT commit a raw credential. Reference `.env` instead "
        "(e.g. \"key: stored in `.env` as FOO_API_KEY\"). "
        "WARN-only — the definitive BLOCK belongs to the pre-commit.\n"
    )
...
def _self_test() -> None:
    ...
    h2 = scan("src/app/foo.py", ["api: sk-ant-abc123DEF456ghi789"], globs)
