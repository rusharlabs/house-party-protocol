...
- Auto-proceed (no confirm-every-step) · plan inline · self-prompt the next item · continuous across batches · workflows in waves of ≤{concurrency.max_agents}.
...
4. **Execute** via workflow waves-of-{concurrency.max_agents} + **adversarial verify** (refute before accepting).
5. **Mark done** in the work-list + commit per path (0 push). An e2e/visual item is done only on `python -m hpp evidence run --id {item}-e2e --artifact "{out}/**/trace.zip" -- {e2e command}` (HPP core; it re-executes — `verify` alone never counts), and its work-list line carries the record path.
...
EXECUTE (waves-of-{concurrency.max_agents} + adversarial verify): order of the charter §. Commit per path, 0 push. Capstone per wave: done_gate --profile {kind}.
