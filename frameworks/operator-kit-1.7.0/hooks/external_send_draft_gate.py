...
_DEFAULT_SEND_TOOLS: list[str] = []  # empty = only external curl/wget POST
...
def _self_test() -> None:
    ...
    r = evaluate("mcp__chat__send_message", {}, ["mcp__chat__send_message"])
    assert r and "external-send" in r, f"should warn on a configured send tool, got {r}"
