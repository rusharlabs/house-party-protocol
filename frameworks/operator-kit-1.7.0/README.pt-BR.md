...
├── agents/                       ← 6 read-only sub-agents: refutador, silent-failure-hunter and
│                                    4 review lenses (lens-verification-gap, lens-partial-set,
│                                    lens-deletion, lens-stale-evidence) answering in hpp.findings/v1
...
Na distribuição emitida este módulo vive em `frameworks/operator-kit-1.7.0/`
...
```bash
KIT=frameworks/operator-kit-1.7.0
...
**Reusar / generalizar / ativar — nunca duplicar (LC-3).** O kit estende o que já existe
em vez de criar paralelos; os mecanismos de loop/verificação
(`ralph_gate`, `determinism_harness`, `passk_eval`, `debt_ledger`) foram construídos a
partir da doutrina já codificada em `rules/` (cada mecanismo é a tradução de uma
regra do próprio kit em código — a sua implementação
literal).

## Lentes de revisão

Cada lente é um sub-agente sem ferramenta de edição que revisa uma mudança atrás de **um** tipo de defeito e
responde com um documento `hpp.findings/v1`: um `code` estável, uma `severity`, `file` e `line`, a
`claim` e a `evidence` em que se apoia, e `inspected`, o universo que ela olhou. O núcleo confere a
forma e deriva o veredito (`python -m hpp findings check FILE...`: exit 0 nada encontrado, 1 algo
encontrado, 2 um documento que quebra o contrato — uma chave que ele não define, um `inspected`
vazio, o mesmo achado duas vezes).

| lente | o que procura |
|---|---|
| `lens-verification-gap` | comportamento que a mudança acrescenta ou altera e que nenhum teste perceberia quebrar |
| `lens-partial-set` | uma mudança aplicada a alguns membros de um conjunto (chamadores, cópias, idiomas, casos) e não aos outros |
| `lens-deletion` | algo removido enquanto código, testes, docs ou configuração ainda dependem dele |
| `lens-stale-evidence` | prova citada para a mudança que é anterior a ela, nomeia outra revisão ou nunca foi rodada de novo |

Elas não têm ferramenta de edição de arquivo; o somente-leitura delas é **provado** quando são sentadas pelo `house_session.py` do módulo de lanes (o worktree ganha impressão digital em volta de cada resposta), e é instrução nos outros casos. Rode-as em paralelo sobre o mesmo diff, cada uma na sua lane; elas são os assentos de um painel de revisão.
...
deles: 13 arquivos de `.claude/rules/*.md` + `docs/HOOK-SKILL-STANDARDS.md`,
...
| `epistemic-standards.md` | Separar FATO (com fonte) de RECOMENDAÇÃO; declarar confiança; nunca inventar | `skills/live-source-prover`, `scripts/live_count.py`; com o núcleo HPP, `python -m hpp cite check` — parcial: ele resolve só marcadores no estilo `[ID:x]`, então os formatos de citação das próprias regras precisam de `--marker`, e um marcador de intervalo é recusado |
| `agent-integrity.md` | Todo conteúdo gerado sobre um agente/persona deve ser rastreável à fonte, nunca inventado | doutrina de referência (sem mecanismo dedicado neste kit); com o núcleo HPP, `python -m hpp cite check` prova que cada marcador resolve — parcial: só marcadores no estilo `[ID:x]`, e não se a fonte sustenta a frase |
...
| `loop-maker-checker.md` | Quem constrói não é quem aprova; checker roda em modelo diferente, read-only | `skills/adversarial-refuter`, `skills/gated-improvement-proposal`; `lane_board.py compete`/`select` (`lane-kit`) quando N construtores competem |
| `loop-passk.md` | pass@k mede capacidade, pass^k mede regressão — 1 run verde não prova nada | `scripts/passk_eval.py`, `scripts/determinism_harness.py`; com o núcleo HPP, `python -m hpp decide eval` e `python -m hpp retrieval eval` |
| `loop-patterns-catalog.md` | Catálogo de 6 arquiteturas de loop autônomo (qual formato usar antes de armar um) | `templates/loop-charter-template.md` (referência ao escolher a forma); `lane_board.py compete`/`select` (`lane-kit`) para escolher 1 de N variações |
...
| `docs/HOOK-SKILL-STANDARDS.md` | Convenções de hook/skill/sub-agent (timeout, exit codes, allowedTools explícito) | aplica-se a todos os hooks/skills deste próprio kit |
