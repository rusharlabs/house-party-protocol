...
In the emitted distribution this module lives in `frameworks/operator-kit-1.7.0/`
...
Each lens is a sub-agent with no file-editing tool that reviews a change for **one** kind of defect and answers
with one `hpp.findings/v1` document: a stable `code`, a `severity`, `file` and `line`, the `claim`
and the `evidence` it rests on, and `inspected`, the universe it looked at. The core checks the
shape and derives the verdict (`python -m hpp findings check FILE...`: exit 0 nothing found, 1
something found, 2 a document that breaks the contract — a key it does not define, an empty
`inspected`, the same finding twice).

| lens | what it looks for |
|---|---|
| `lens-verification-gap` | behaviour the change adds or alters that no test would notice breaking |
| `lens-partial-set` | a change applied to some members of a set (callers, copies, languages, cases) and not the others |
| `lens-deletion` | something removed while code, tests, docs or configuration still depend on it |
| `lens-stale-evidence` | proof cited for the change that predates it, names another revision, or was never re-run |

They carry no file-editing tool; their read-only is **proved** when they are seated through the lane module's `house_session.py` (the worktree is fingerprinted around each answer), and is an instruction otherwise. Run them in parallel on the same diff, each in its own lane; they are the seats of a review panel.
