...
An end-to-end case runs through `hpp evidence run` (requires the HPP core, `python -m hpp`), so
each of the `k` runs leaves its own record and a flaky run leaves the trace that shows why:

```json
{
  "id": "e2e-login",
  "runner": "command",
  "command": ["python", "-m", "hpp", "evidence", "run", "--id", "e2e-login", "--timeout", "540",
              "--artifact", "pw-out/**/trace.zip", "--", "npx", "playwright", "test", "tests/login.spec.ts"],
  "expect_exit": 0,
  "timeout_seconds": 600
}
```

End-to-end runners often wipe their output folder when a run starts, so keep each run's traces
in a folder of its own when they must stay on disk; the records keep the hashes either way. Do
not hunt e2e flakes with a stdout-hashing determinism harness (`determinism_harness.py`): timing
and ordering change the output of runs that all passed. Read pass^k on exit codes instead.

## Choosing the verifier

Use the most deterministic verifier that measures the real behavior:

1. code: test, schema, exit code or checksum; retrieval quality → `python -m hpp retrieval eval`
   (HPP core);
2. rule: format, presence, absence or explicit pattern;
3. model: rubric for open-ended quality, assuming variation of the judge itself; record the
   verdict as `hpp.decision/v1` (advisory, raise-only) and measure the decider first with
   `python -m hpp decide eval`. It may raise a block, never grant a pass;
...
- a best-of-N winner read as reliability — choosing 1 of N is pass@N; the winner still needs pass^k.
