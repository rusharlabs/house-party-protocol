...
**What this kit does NOT do:** it does not ship the tree of
...
**In-kit fallbacks (optional, they need the HPP core):** a few checks do not depend on that tree.
When their task file is absent, `*evidence-check` (`*qa`) runs `python -m hpp evidence verify` on
every record the story lists in Debug Log References; `*console-check` (`*qa`) and every mode of
`*verify-subtask` (`*dev`) run their criterion through `python -m hpp evidence run`, which measures
the exit code and hashes the declared artifacts (a trace, a console log); `*pre-push` (`*devops`)
blocks unless every end-to-end record the story lists verifies; and `pp-consolidate` runs
`python -m hpp cite check` over its verdict. A browser agent is a maker/exploration tool: a verdict
is the exit code of a versioned spec, recorded as evidence. If the HPP core is absent, those commands
say so instead of skipping silently.
...
| HPP core (`python -m hpp`) | 2.6.0 or newer (the first release with `hpp evidence` and `hpp cite check`) | optional — only for the in-kit fallbacks (§1) |
| Node + an end-to-end runner (e.g. Playwright) | — | optional — only for the browser/e2e checks |
...
`frameworks/dev-squad-kit-1.1.1/` (the directory carries the version — state it
...
installing. One step is the exception: `pp-consolidate` ends with `python -m hpp cite check`, a real
exit code from the HPP core (its executed example shows a block on a marker that does not resolve).
