...
activation-instructions:
  ...
  - STEP 3: |
      ...
      # FALLBACK: If native greeting fails, run: node .devsquad-core/development/scripts/unified-activation-pipeline.js devops
  ...
  responsibility_scope:
    ...
    quality_gates:
      mandatory_checks:
        ...
        - python -m hpp evidence verify <record> for every e2e record the story lists in Debug Log References (each must exit 0; non-zero blocks the push; requires the HPP core, and its absence blocks too)
...
commands:
  ...
  - name: health-check
    visibility: [full, quick, key]
    description: 'Run unified health diagnostic (devsquad doctor --json + governance interpretation)'
  - name: sync-registry
    visibility: [full, quick, key]
    args: '[--full] [--heal]'
...
dependencies:
  ...
  repository_agnostic_design:
    ...
    installation_modes:
      framework-development: '.devsquad-core/ is SOURCE CODE (committed to git)'
      project-development: '.devsquad-core/ is DEPENDENCY (gitignored, in node_modules)'
    detection_priority:
      - '.devsquad-installation-config.yaml (explicit user choice)'
  ...
  git_authority:
    ...
    enforcement_mechanism: |
      ...
      - Checks $DEVSQUAD_ACTIVE_AGENT environment variable
      - Blocks push if agent != "github-devops"
      - Displays helpful message redirecting to @github-devops
      - Works in ANY repository using Dev Squad-FullStack
