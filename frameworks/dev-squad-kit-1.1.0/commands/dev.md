...
>
> **Exception — in-kit fallback:** `*verify-subtask` is not skipped when its task file is absent:
> each mode (command, api, browser, e2e) falls back to `python -m hpp evidence` (the HPP core), as
> the command's `fallback` below says. If the HPP core is also absent (`python -m hpp --version`
> fails), say so and do not mark the subtask verified — never silently.
...
activation-instructions:
  ...
  - STEP 3: |
      ...
      # FALLBACK: If native greeting fails, run: node .devsquad-core/development/scripts/unified-activation-pipeline.js dev
  ...
  - STAY IN CHARACTER!
  - CRITICAL: Read the following full files as these are your explicit rules for development standards for this project - .devsquad-core/core-config.yaml devLoadAlwaysFiles list
...
commands:
  ...
  - name: verify-subtask
    ...
    fallback: |
      When verify-subtask.md is absent (requires the HPP core), record the mode as evidence:
        command: python -m hpp evidence run --id <story>-<subtask> [--artifact <file it must produce>] -- <argv>
        api:     python -m hpp evidence run --id <story>-<subtask> --artifact <probe-output> -- python <probe>.py
                 (a stdlib Python probe that calls the endpoint, writes <probe-output> and exits
                 non-zero on a wrong status or body; not curl, which the HPP command policy
                 classifies MANUAL)
        browser: python -m hpp evidence run --id <story>-<subtask> --artifact "<out>/**/trace.zip" -- npx playwright test <spec> --output <out> --trace on
        e2e:     python -m hpp evidence run --id <story>-<subtask> --artifact "<out>/**/trace.zip" -- npx playwright test <e2e-spec> --output <out> --trace on
      browser and e2e also need Node and an end-to-end runner; <out> is a fresh per-run directory
      inside the workspace. Exit 0 = verified: write the record path (.hpp/evidence/<id>-<UTC>.json)
      into Debug Log References. Exit 1 = not verified (the record's verdict says why); exit 2 =
...
develop-story:
  ...
  completion: "All Tasks and Subtasks marked [x] and have tests→Validations and full regression passes (DON'T BE LAZY, EXECUTE ALL TESTS and CONFIRM — CONFIRM = the path of a passed `hpp evidence run` record written into Debug Log References, which @qa verifies with `python -m hpp evidence verify`)→Ensure File List is Complete→run the task execute-checklist for the checklist story-dod-checklist→set story status: 'Ready for Review'→HALT"
...
dependencies:
  ...
  tasks:
    ...
    - verify-subtask.md # ADE: Verify subtask completion (command, api, browser, e2e) (resolved via hpp evidence when absent)
  ...
  coderabbit_integration:
    ...
    workflow: |
      ...
      WHILE iteration < max_iterations:
        1. Run: wsl bash -c 'cd /mnt/c/.../devsquad-core && ~/.local/bin/coderabbit --prompt-only -t uncommitted'
  ...
  decision_logging:
    ...
    utility: '.devsquad-core/utils/decision-log-generator.js'
    ...
    usage_example: |
      ...
      const { generateDecisionLog } = require('.devsquad-core/utils/decision-log-generator');
