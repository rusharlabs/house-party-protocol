...
**O que este kit NÃO faz:** não inclui a árvore de tasks/templates/checklists
...
**Fallbacks dentro do kit (opcionais, precisam do núcleo HPP):** algumas verificações não dependem
dessa árvore. Quando o arquivo de task não existe, `*evidence-check` (`*qa`) roda
`python -m hpp evidence verify` em cada registro que a story lista em Debug Log References;
`*console-check` (`*qa`) e todos os modos de `*verify-subtask` (`*dev`) rodam o critério por
`python -m hpp evidence run`, que mede o exit code e faz o hash dos artefatos declarados (um trace,
um log de console); `*pre-push` (`*devops`) bloqueia se algum registro end-to-end que a story lista
não verificar; e `pp-consolidate` roda `python -m hpp cite check` sobre o veredito. Um agente de
navegador é ferramenta de maker/exploração: o veredito é o exit code de uma spec versionada,
registrado como evidência. Se o núcleo HPP não estiver presente, esses comandos dizem isso em vez de
pular em silêncio.
...
| Núcleo HPP (`python -m hpp`) | 2.6.0 ou mais nova (a primeira release com `hpp evidence` e `hpp cite check`) | opcional — só para os fallbacks do kit (§1) |
| Node + um runner end-to-end (ex.: Playwright) | — | opcional — só para as verificações browser/e2e |
...
`frameworks/dev-squad-kit-1.1.0/` (o diretório carrega a versão — declare-a
uma vez, em `KIT`). O instalador é `installers/kit-forge-1.4.2/kit_doctor.py`; rode-o da
raiz da distribuição. Ele planeja primeiro e só escreve numa segunda invocação explícita
com `--apply`:

```bash
KIT=frameworks/dev-squad-kit-1.1.0
...
```bash
python installers/kit-forge-1.4.2/tools/skill_lint.py --all frameworks/dev-squad-kit-1.1.0/skills --run-proofs
```
...
serve seu caso antes de instalar. Um passo é a exceção: `pp-consolidate` termina com
`python -m hpp cite check`, um exit code real do núcleo HPP (o exemplo executado dela mostra um
bloqueio num marcador que não resolve).
