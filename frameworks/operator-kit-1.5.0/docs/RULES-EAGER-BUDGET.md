...
**Per-session cost fell 65,8 %** (165.955 → 56.770 B; it was 55.754 B before two eager rules took the Symphony doctrine on the same day). The layer on disk *grew* by 6.677 B in
...
| `loop-maker-checker` | 11.682 | Gates push / merge / closing a goal — moments reachable from anywhere, and irreversible once passed. |
...
| `stale-replay-guard` | 5.452 | Triggers on `/resume` and context restore — events with no file path to hang a glob off. |
...
| `loop-patterns-catalog` | 19.879 | loop / cron / squad paths, `.claude/commands/**` | A design-time catalogue ("which shape?"), consulted once per loop, not once per session. |
| `loop-operator` | 16.391 | loop / cron / squad paths, `.claude/commands/**` | Pre-flight and stop-conditions in full detail; the always-on half of loop safety is `loop-cost-budget`, which stayed eager. |
| `partial-autonomy-slider` | 8.367 | agents, `operator-profile.yaml`, `_autonomy-registry.yaml` | It governs a value that lives in those files; reading it elsewhere changes nothing. |
| `loop-passk` | 3.720 | evals, loop paths, `_autonomy-registry.yaml` | pass@k / pass^k applies when you are running an eval or promoting an agent. |
...
`tests/python/test_kits/test_rules_eager_budget.py` holds the number:
...
- **budget** — eager bytes may not pass 59.000. The headroom above 56.770 is smaller than the
  smallest scoped rule (3.720 B), so no rule can quietly slip back into the eager set, and a
...
```bash
python -X utf8 -m pytest tests/python/test_kits/test_rules_eager_budget.py -q
```
