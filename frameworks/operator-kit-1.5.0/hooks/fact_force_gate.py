...
Adapted from ECC (MIT) — the first-touch idea, the session-state shape and the
parallel-batch lesson. No code copied; this is a Python reimplementation.

v1.0.0 — 2026-09-22 (Operator Kit · A1 · first-touch fact gate)
"""
...
def save_state(path: Path, state: dict) -> bool:
    ...
    try:
        ...
        # would deny every Edit in the session (the infinite-deny the ECC autopsy warns about).
        return False
