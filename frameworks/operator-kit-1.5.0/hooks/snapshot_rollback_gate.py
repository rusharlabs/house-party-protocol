...
def _warn(hits: list[str]) -> None:
    ...
    sys.stderr.write(
        f"[snapshot_rollback_gate] WARNING: destructive action detected ({', '.join(uniq)}).\n"
        "  Before running it, confirm the SNAPSHOT + ROLLBACK + VERIFY protocol:\n"
        "    [ ] Was there a SNAPSHOT/backup of what will be destroyed?\n"
        "    [ ] Is there a ROLLBACK PLAN if it goes wrong?\n"
        "    [ ] Is there a post-action VERIFY step (test that the system is still OK)?\n"
        "  If all 3 are covered, set SNAPSHOT_CONFIRMED=1 to silence this. "
