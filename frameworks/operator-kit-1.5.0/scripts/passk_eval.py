...
def _load_suite(path: Path) -> tuple[dict[str, Any], str]:
    try:
        ...
        raise SuiteError(f"unreadable suite: {path}: {exc}") from exc
    if not isinstance(suite, dict):
        raise SuiteError("the suite must be a JSON object")
    if not isinstance(suite.get("suite"), str) or not suite["suite"].strip():
        raise SuiteError("the suite requires a non-empty name in 'suite'")
    if not isinstance(suite.get("version"), str) or not suite["version"].strip():
        raise SuiteError("the suite requires a string version in 'version'")
    cases = suite.get("cases")
    if not isinstance(cases, list) or not cases:
        raise SuiteError("the suite requires a non-empty list in 'cases'")
    ids = [case.get("id") for case in cases if isinstance(case, dict)]
    if len(ids) != len(cases) or any(not isinstance(cid, str) or not cid for cid in ids):
        raise SuiteError("every case requires a non-empty string id")
    duplicated = sorted({cid for cid in ids if ids.count(cid) > 1})
    if duplicated:
        raise SuiteError(f"duplicate case id: {duplicated}")
...
def _replay(case: dict[str, Any], k: int) -> list[bool]:
    ...
    if not isinstance(outcomes, list) or len(outcomes) < k:
        raise SuiteError(f"{case['id']}: replay requires at least {k} outcomes")
    if any(type(value) is not bool for value in outcomes[:k]):
        raise SuiteError(f"{case['id']}: outcomes must be booleans")
    return outcomes[:k]
...
def _command(case: dict[str, Any], k: int, cwd: Path) -> list[bool]:
    ...
    if not isinstance(command, list) or not command or any(not isinstance(x, str) for x in command):
        raise SuiteError(f"{case['id']}: command must be a non-empty JSON argv")
    expected_exit = case.get("expect_exit", 0)
    if not isinstance(expected_exit, int):
        raise SuiteError(f"{case['id']}: expect_exit must be an integer")
    timeout = case.get("timeout_seconds", 30)
    if not isinstance(timeout, (int, float)) or timeout <= 0:
        raise SuiteError(f"{case['id']}: timeout_seconds must be positive")
    ...
    if expected_out is not None and not isinstance(expected_out, str):
        raise SuiteError(f"{case['id']}: expect_stdout_contains must be a string")
    if expected_err is not None and not isinstance(expected_err, str):
        raise SuiteError(f"{case['id']}: expect_stderr_contains must be a string")
    ...
    for _ in range(k):
        try:
            ...
            raise SuiteError(f"{case['id']}: command not executable: {exc}") from exc
...
def run_suite(path: Path, *, k: int = DEFAULT_K) -> PassKReport:
    ...
    for case in suite["cases"]:
        ...
        if runner == "replay":
            ...
            raise SuiteError(f"{case['id']}: invalid runner: {runner!r}")
...
def evaluate_gate(
    ...
    *,
    ...
    if not 0 <= passk_threshold <= 1:
        raise SuiteError("passk_threshold must be between 0 and 1")
    ...
    if gate == "capability":
        ...
        raise SuiteError("invalid gate; use capability, regression or both")
    return {
        "gate": gate,
        "passed": passed,
...
def _self_test() -> int:
    ...
    print("self-test OK — standalone runner, replay, pass@k, pass^k and gates")
    return 0
