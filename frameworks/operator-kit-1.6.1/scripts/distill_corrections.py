...
Mines session logs for recurring operator corrections and proposes them as rules for
`distill.target_rule` (default learned-corrections.md): reads the last N session logs from `paths.logs_dir`
...
def _strip_log_prefix(line: str) -> str:
    ...
    s = re.sub(r"^(user|usuario|operador)\s*[:>-]\s*", "", s, flags=re.IGNORECASE)
    return s.strip()
