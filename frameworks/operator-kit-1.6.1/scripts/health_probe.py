...
WITHOUT touching the network, and prints a summary. Meant to run at SessionStart / cron /
on demand -- NEVER in the statusline's path (which only reads the cache).

Probe types (health.probes = list of {name, type, target}):
  http -> urllib GET; online if status < 400 (NEVER uses curl/wget -- deny-list)
  cmd  -> subprocess shell; online if exit 0 (command comes from the profile = trusted)
...
exit 0 always (the connector doesn't break the flow). stdlib + PyYAML (via loader).
v1.1.0 -- 2026-07-10 (Operator Kit -- probes and cache keys named services/health)
"""
...
def probe_http(target: str, timeout: int = 5) -> bool:
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout) as r:  # noqa: S310 -- target from the profile
...
def probe_cmd(target: str, root: Path, timeout: int = 10) -> bool:
    try:
        r = subprocess.run(target, shell=True, cwd=str(root),  # noqa: S602 -- command from the profile (trusted)
...
def main(argv) -> int:
    ...
    if not probes:
        if not quiet:
            print("health_probe: no probe in health.probes of operator-profile.yaml", file=sys.stderr)
        return 0
...
def _self_test() -> None:
    ...
    assert _probes_cfg() == _probes_cfg()  # idempotent / doesn't crash
    print("self-test OK")
