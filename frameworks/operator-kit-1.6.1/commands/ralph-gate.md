...
argument-hint: "--goal G-X | --charter \"prompt\" --criteria \"cmd1\" [\"cmd2\" ...] [--max-iterations N] [--criterion-timeout S]"
...
Unlike a loop that trusts the text, the `<promise>` only releases the stop if `done_gate.py`
...
Each criterion may run for `--criterion-timeout` seconds (default 20). The Stop hook's own `timeout`
(120 s in `hooks/hooks.json`) must cover the sum of them — raise both for long criteria. For an
end-to-end or visual item the criterion is `python -m hpp evidence run ...` (requires the HPP core):
it re-executes the spec. `hpp evidence verify` alone is never the latch — it only re-hashes a record
that was written before, and a record is self-hashed, not signed.
