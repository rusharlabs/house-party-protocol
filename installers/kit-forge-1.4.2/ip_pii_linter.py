...
stdlib + PyYAML. Cross-platform (pathlib).
...
_SELFTEST_RULESET = {
    ...
    "derived": {
        ...
        "content": ["thirdpartyforkxyz", "genericrefterm"],
        "reference_only_ok": ["genericrefterm"],
...
def _self_test() -> int:
    ...
    try:
        ...
        (tmp / "derived_ref_noattrib.txt").write_text("based on genericrefterm pattern\n", encoding="utf-8")
        (tmp / "derived_ref_attrib.txt").write_text(
            "based on genericrefterm pattern\n# MIT-ATTRIBUTION-TAG\n", encoding="utf-8"
        )
        ...
        # secret-shaped literal, and the split survives only in the source -- the runtime value is intact.
        ...
        assert "derived.attribution_missing" in by_rule, "reference-only term without attribution should block"
        ref_attrib_findings = [f for f in findings if f["file"] == "derived_ref_attrib.txt"]
        assert not ref_attrib_findings, f"reference-only term with attribution should not produce a finding: {ref_attrib_findings}"
