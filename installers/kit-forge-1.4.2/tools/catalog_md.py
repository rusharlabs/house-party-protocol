...
#
# Why: the path was `../assets/`, and Pages serves
# `docs/` as the ROOT -- `../` leaves the published root, so the logo returned 503 on all five
# pages. No test saw it, because tests read the file from disk, where `..` resolves.
# `assets/` with a copy of the asset inside `docs/` works in BOTH contexts.
...
def build_model(root: Path) -> dict:
    ...
    return {
        ...
        # Why: the landing page linked the four pages and the publisher, but NOT
        # the code, so a visitor arriving from a shared link had no way to the repository. The
        # address comes from the manifest for the same reason as the publisher's: a literal in
        # a generator that ships inside every kit outlives the address it names.
...
def render_index(model: dict) -> str:
    ...
    # Why: without a card, a link to the site shared on a social network shows no
    # title and no image. The absolute address comes from the manifest: a crawler does not
    # resolve relative paths, and a literal here would outlive the address it names.
    site = model.get("site", "")
    if site:
...
def write_fixture_tree(root: Path) -> None:
    ...
    (kit / "agents" / "NOTICE-UPSTREAM.md").write_text("# notice\n", encoding="utf-8")
    (kit / "docs").mkdir()
...
def _self_test() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for md in (pt, en):
            ...
            assert "`checker`" in md and "NOTICE-UPSTREAM" not in md
