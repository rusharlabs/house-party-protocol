...
An existing `.git/hooks/pre-commit` can be an important wrapper; a naive installer would
replace that protection and create a silent regression. This installer never overwrites:
it chains.
...
stdlib only. v1.0.0 -- 2026-07-10 (PHASE 1 - kit-forge)
"""
...
def install(repo: Path, hook_name: str, payload_path: Path) -> tuple:
    ...
    if target.exists():
        ...
        if _MARKER in existing_text:
            # already chained by us -- update only the payload if it changed, idempotent on the rest
...
def _self_test() -> int:
    ...
    try:
        ...
        # scenario 2: someone else's hook already exists -- should chain, not replace
        repo2 = tmp / "repo2"
