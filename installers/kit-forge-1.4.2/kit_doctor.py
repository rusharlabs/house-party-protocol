...
def check_marketplace(root: Path) -> dict:
    ...
    The canonical manifest is `<root>/.claude-plugin/marketplace.json`, never
    `<root>/marketplace.json` alone: `/plugin marketplace add` looks for it there. The
    relative `source` values resolve from the marketplace ROOT, not from inside
    `.claude-plugin/` (e.g. `./plugins/<x>` points at `<root>/plugins/<x>`).
    """
    root = Path(root)
    report: dict = {
    ...
    for entry in plugins:
        ...
        if not pj.exists():
            # Why: a plugin dir may legitimately ship only LICENSE+README, with the entire
            # metadata living in the marketplace entry. Failing it would reject valid catalogues,
            # and a gate that yells at the innocent gets turned off before it is ever right.
            # WARN: the version cannot be cross-checked.
            _finding(
                "kit-sem-plugin-json",
        ...
        try:
            ...
            continue
        # Why: `version` is OPTIONAL in the marketplace entry, and many valid entries omit it.
        # Comparing `None != "1.2.1"` would fail the whole catalogue. Whoever does not declare
        # it cannot diverge: there the plugin.json is the single source.
